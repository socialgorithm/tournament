...
var SocketServer = (function () {
    function SocketServer(port, gameServers) {
        ...
        this.addPlayerToNamespace = function (data) {
            ...
            _this.playerSockets[data.player].socket.join(data.namespace);
            _this.playerSockets[data.player].emit(new model_1.Events.GameListEvent(_this.gameServers.map(function (server) { return server.status; })));
        };
        ...
        this.sendMessageToPlayer = function (data) {
            var socket = _this.playerSockets[data.player].socket;
            if (!socket) {
        ...
        this.sendGameListToEveryone = function (data) {
            ...
            Object.values(_this.playerSockets).forEach(function (socket) {
                socket.emit(new model_1.Events.GameListEvent(data));
            });
        };
    ...
    SocketServer.prototype.start = function () {
        ...
        this.io.on("connection", function (rawSocket) {
            ...
            var token = rawSocket.handshake.query.token;
            var player = token;
            ...
            listenToEvents.forEach(function (event) {
                rawSocket.on(event, _this.onMessageFromSocket(player, event));
            });
            rawSocket.on("disconnect", _this.onPlayerDisconnect(player));
        });
