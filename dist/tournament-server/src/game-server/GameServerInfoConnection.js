...
var GameServerInfoConnection = (function () {
    function GameServerInfoConnection(gameServerAddress) {
        ...
        this.gameSocket = new model_1.SocketClient(gameServerAddress);
        this.gameSocket.socketIOSocket.on("connect", function () {
            debug("Connected to " + gameServerAddress);
            _this.status.healthy = true;
            _this.publishStatus();
        });
        this.gameSocket.socketIOSocket.on("connect_failed", function () {
            debug("Connection to " + gameServerAddress + " failed");
            _this.status.healthy = false;
            _this.publishStatus();
        });
        this.gameSocket.socketIOSocket.on("error", function () {
            debug("Connection to " + gameServerAddress + " errored");
            _this.status.healthy = false;
            _this.publishStatus();
        });
        this.gameSocket.addHandler(new model_1.Handlers.GameInfoEventHandler(function (gameInfo) {
            _this.status.info = gameInfo;
            _this.publishStatus();
        }));
        this.gameSocket.socketIOSocket.on("disconnect", function () {
