...
export default class DoubleEliminationMatchmaker implements IMatchmaker {
  ...
  public updateStats(playedMatches: Match[]): void {
      ...
      if (match.winner === RESULT_TIE) {
        matches.push(
          this.createMatch(
            match.players[0],
            match.players[1],
            { timeout: match.options.timeout / 2, maxGames: match.options.maxGames, autoPlay: match.options.autoPlay },
  ...
  private createMatch(playerA: Player, playerB: Player, optionOverrides?: MatchOptions, parentMatches?: MatchParent[]): DoubleEliminationMatch {
